...
A1.1,B2.1,C3.1,A1.1,Single,eYFPG5A,85.1,300
A2.1,B3.1,C4.1,A2.1,Single,mKO2,50,300
A3.1,B4.1,C5.1,A3.1,Single,tagBFP,100,300
A1.1,B5.1,C6.1,A4.1,Co,eYFPG5A,85.1,166.6666667
A2.1,B5.1,C6.1,A4.1,Co,mKO2,50,166.6666667
A3.1,B5.1,C6.1,A4.1,Co,tagBFP,100,166.6666667
A4.1,B6.1,D1.1,B1.1,Co,0u+ to 1u- : epegRNA,56.4,100
A5.1,B6.1,D1.1,B1.1,Co,0u+ to 1u- : PEmax,50,300
A6.1,B6.1,D1.1,B1.1,Co,0u+ to 1u- : 2ndary cut,50,100
A2.1,B6.1,D1.1,B1.1,Co,0u+ to 1u- : mKO2,50,100
A4.1,B6.1,D1.1,B2.1,Co,0u+ to 1u- : epegRNA,56.4,100
A5.1,B6.1,D1.1,B2.1,Co,0u+ to 1u- : PEmax,50,300
A6.1,B6.1,D1.1,B2.1,Co,0u+ to 1u- : 2ndary cut,50,100
A2.1,B6.1,D1.1,B2.1,Co,0u+ to 1u- : mKO2,50,100
A4.1,B6.1,D1.1,B3.1,Co,0u+ to 1u- : epegRNA,56.4,100
A5.1,B6.1,D1.1,B3.1,Co,0u+ to 1u- : PEmax,50,300
A6.1,B6.1,D1.1,B3.1,Co,0u+ to 1u- : 2ndary cut,50,100
A2.1,B6.1,D1.1,B3.1,Co,0u+ to 1u- : mKO2,50,100
B1.1,C1.1,D2.1,B4.1,Co,0u+ to 1u- : NO epegRNA,100,100
A5.1,C1.1,D2.1,B4.1,Co,0u+ to 1u- : PEmax,50,300
A6.1,C1.1,D2.1,B4.1,Co,0u+ to 1u- : 2ndary cut,50,100
A2.1,C1.1,D2.1,B4.1,Co,0u+ to 1u- : mKO2,50,100
B1.1,C1.1,D2.1,B5.1,Co,0u+ to 1u- : NO epegRNA,100,100
A5.1,C1.1,D2.1,B5.1,Co,0u+ to 1u- : PEmax,50,300
A6.1,C1.1,D2.1,B5.1,Co,0u+ to 1u- : 2ndary cut,50,100
A2.1,C1.1,D2.1,B5.1,Co,0u+ to 1u- : mKO2,50,100
B1.1,C1.1,D2.1,B6.1,Co,0u+ to 1u- : NO epegRNA,100,100
A5.1,C1.1,D2.1,B6.1,Co,0u+ to 1u- : PEmax,50,300
A6.1,C1.1,D2.1,B6.1,Co,0u+ to 1u- : 2ndary cut,50,100
A2.1,C1.1,D2.1,B6.1,Co,0u+ to 1u- : mKO2,50,100
B1.1,C2.1,D3.1,A5.1,Single,Transfection ctrl,500,500'''
...
DNA_sources_, DNA_dests_, L3K_dests_, plate_dests,transfection_types, tube_names, uL_DNA_, uL_OM_, uL_P3K_, uL_L3K_ = [],[],[],[],[],[],[],[],[],[]
...
def run(protocol: protocol_api.ProtocolContext):
    ...
    count = 0
    while count < len(DNA_dests_):
        destination_well = DNA_dests_[count].split('.')[0]
        dest_rack = DNA_dests_[count].split('.')[-1]
        ...
        if trans_type == 'Co':            
            ...
            for b in range(count+1,len(DNA_dests_)):
                if (DNA_dests_[count].split('.')[0] == DNA_dests_[b].split('.')[0]) and (L3K_dests_[count].split('.')[0] == L3K_dests_[b].split('.')[0]):
                    cotrans_group_indices.append(b)

            OM_P3K_MM_vol = 0            
            for c in cotrans_group_indices:
                OM_P3K_MM_vol += (uL_OM_[c]+uL_P3K_[c])
            
            count += len(cotrans_group_indices)        

        else:
            OM_P3K_MM_vol = (uL_OM_[count]+uL_P3K_[count])
    ...
    count = 0
    while count < len(L3K_dests_):
        destination_well = L3K_dests_[count].split('.')[0]
        dest_rack = L3K_dests_[count].split('.')[-1]
        ...
        if trans_type == 'Co':            
            ...
            OM_L3K_MM_vol = 0            
            for c in cotrans_group_indices:
                OM_L3K_MM_vol += (uL_OM_[c]+uL_L3K_[c])
            
            count += len(cotrans_group_indices)        

        else:
            OM_L3K_MM_vol = (uL_OM_[count]+uL_L3K_[count])
    ...
    count = 0
    while count < len(L3K_dests_):
        source_well = DNA_dests_[count].split('.')[0]
        destination_well = L3K_dests_[count].split('.')[0]
        source_rack = DNA_dests_[count].split('.')[-1]
        dest_rack = L3K_dests_[count].split('.')[-1]
        ...
        if trans_type == 'Co':            
            ...
            mixing_vol = 0            
            for c in cotrans_group_indices:
                mixing_vol += (uL_DNA_[c]+uL_OM_[c]+uL_P3K_[c])
            
            count += len(cotrans_group_indices)        

        else:
            mixing_vol = (uL_DNA_[count]+uL_OM_[count]+uL_P3K_[count])
    ...
    while count < len(plate_dests):
        ...
        if trans_type == 'Co':            
            ...
            cotrans_group_indices = [count]
            for b in range(count+1,len(DNA_dests_)):
                if (DNA_dests_[count].split('.')[0] == DNA_dests_[b].split('.')[0]) and (plate_dests[count].split('.')[0] == plate_dests[b].split('.')[0]):
                    cotrans_group_indices.append(b)
